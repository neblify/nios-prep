    ...
    return (
        <div className="space-y-8">
            <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
                ...
                <p className="text-gray-500 mt-2">{test.subject} • {test.questions.length} Questions</p>
            </div>

            <div className="space-y-6">
                {test.questions.map((q: IQuestion, index: number) => (
                                    ...
                                    <div className="space-y-2">
                                        {q.options?.map((opt, i) => (
                                            <label key={i} className="flex items-center gap-3 p-3 rounded-lg border border-gray-200 cursor-pointer hover:bg-gray-50 transition-colors">
                                                <input
                                                    type="radio"
                                                    name={`q-${index}`} // Using index as key might be unstable if reordered, but fine here
                                                    value={opt}
