...
test.describe('Teacher Flow - Create and Publish Test', () => {
    test('should login as teacher, create a test, and publish it', async ({ page }) => {
        ...
        // Title
        await page.getByPlaceholder('e.g. Science Mid-Term').fill('E2E Automated Test');

        // Subject
        await page.getByPlaceholder('e.g. Mathematics').fill('Math');

        // Step 4: Add a question
        await page.getByRole('button', { name: /Add Question to Section A/i }).click();

        // Wait for question form to appear
        await page.waitForTimeout(1000);

        // Fill question text
        await page.locator('textarea[placeholder^="Question"]').fill('What is 2+2?');
        ...
        await page.locator('input[placeholder="Correct Option (Exact Match)"]').fill('4');

        // Step 5: Publish the test
        const publishButton = page.getByRole('button', { name: /Update Test|Publish/i });
